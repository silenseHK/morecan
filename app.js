...
App({
  ...
  onStartupScene(query) {
    ...
    let invitationCode = query.invitation_code ? query.invitation_code : scene.uid;
    refereeId > 0 && (this.saveRefereeId(refereeId));
    invitationCode > 0 && (this.saveRefereeInvitationCode(invitationCode))
  },
 ...
 /**
   * 记录邀请码
   */
  saveRefereeInvitationCode(invitationCode) {
    if (!wx.getStorageSync('invitation_code'))
  ...
  getUserInvitationCode() {
    return wx.getStorageSync('invitation_code');
  },
  /**
   * 获取用户邀请码
   */
  ...
  */
  ...
  _post_form_bindPhone(url, data, success, fail, complete, isShowNavBarLoading) {
    let _this = this;
  ...
  getShareUrlParams(params) {
    ...
    return util.urlEncode(Object.assign({
      ...
      // invitation_code: _this.getUserInvitationCode()
    }, params));
  },
